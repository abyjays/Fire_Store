export default function Userindex() {
  ...
  const handleAddToCart = (product) => {
    ...
    if (existingIndex >= 0) cart[existingIndex].cartQuantity = (cart[existingIndex].cartQuantity || 1) + 1;
    else cart.push({ ...product, cartQuantity: 1, userEmail: user.email });
        ...
        {products.length > 0 ? (
          products.map((p, i) => (
            <div key={i} className="product-card">
              <div>
                <h3>{p.productname}</h3><p className="price">₹{p.productprice}</p><p className="stock">Stock Available: {p.quantity}</p>
              </div>
