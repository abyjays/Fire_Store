...
export default function AdminProduct() {
  const [product, setProduct] = useState({ productname: "", productprice: "", quantity: "", image: "" });
  ...
  const handleAdd = (e) => {
    ...
    setProduct({ productname: "", productprice: "", quantity: "", image: "" });
  };
  ...
  const handlePriceChange = (index, value) => {
    const updated = [...products]; 
    updated[index].productprice = value; 
    setProducts(updated); 
    localStorage.setItem("Products", JSON.stringify(updated));
  };
    ...
    <div className="container">
      ...
      <div className="form-card" style={{ maxWidth: "800px", margin: "0 auto 2rem" }}>
        ...
        <form className="admin-add-form" onSubmit={handleAdd} style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '1rem' }}>
          ...
          <input type="text" name="image" placeholder="Image URL (Optional)" value={product.image} onChange={handleChange} />
      ...
      <div className="table-responsive">
        {products.length > 0 ? (
          <table>
            <thead><tr><th>#</th><th>Image</th><th>Name</th><th>Edit Price (₹)</th><th>Edit Qty</th><th>Actions</th></tr></thead>
            <tbody>
              {products.map((p, i) => (
                <tr key={i}>
                  <td>{i + 1}</td>
                  ...
                  <td>
                    <input type="number" value={p.quantity} onChange={(e) => handleQuantityChange(i, e.target.value)} style={{ width: "70px", padding: "0.4rem" }} />
                  </td>
                  <td>
